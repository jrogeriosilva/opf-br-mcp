...
- `src/core/filter-sets.ts` — `FILTER_SETS`: filter specs shared verbatim by a whole family of domains (`openapi`, `sections`). Domains spread them into their own `filters` (still the single source `search` validates against) and set `filterSet`; `list_domains` emits each set once instead of repeating it per domain.
...
- `search` results can be summaries (e.g. `payments-v5-openapi` strips the `detail` node); `get_item` returns the full record. Item ids are stable and only discoverable via `search` — tool descriptions tell the client agent this.
- Domain fetchers are polite to sources: inter-request delay (Confluence) and retry backoff come from `config.ts`, not hardcoded.
- Domains wrapping a versioned spec must encode the spec's major version in the domain id, e.g. `payments-v5-openapi` for Payments API v5 (`specVersion` in `config.ts`). When the source publishes a new major, add a new domain rather than mutating the existing id. Remove the previous major separately only when explicitly requested after its discontinuation — this keeps ids stable while a version remains supported.
...
- One source also splits into paired domains when part of it has a different **item shape** — prose sections vs. a record table. That is what separates `pcm-business-rules` from `pcm-additional-info`, and `requisitos-nao-funcionais` (the normative text) from `limites-por-endpoint` (one record per endpoint). The trigger is concrete: run `parseSections` on the page first; if the headings come back empty because the tables are siblings of an ancestor heading, sections is the wrong shape — write a table parser and give it its own id, naming the record (`limites-por-endpoint`), not the source page (`referencia`). Cross-reference both descriptions.
